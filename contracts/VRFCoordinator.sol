...
contract VRFCoordinator is VRF, VRFRequestIDBase {
    ...
    HashBridgeOracleTokenInterface internal HBO;
    BlockHashStoreInterface internal blockHashStore;

    constructor(address _HBO, address _blockHashStore) public {
    ...
    struct Callback {
        ...
        // Amount of HBO paid at request time. Total HBO = 1e9 * 1e18 < 2^96, so
    ...
    struct ServiceAgreement {
        ...
        uint96 fee; // Minimum payment for oracle response. Total HBO=1e9*1e18<2^96
        bytes32 jobID; // ID of corresponding HashBridgeOracle job in oracle's DB
    }
     ...
     * @param _fee minimum HBO payment required to serve randomness
     * @param _oracle the address of the HashBridgeOracle node with the proving key and job
     * @param _publicProvingKey public key used to prove randomness
     * @param _jobID ID of the corresponding HashBridgeOracle job in the oracle's db
     */
    function registerProvingKey(
        ...
        require(
            _fee <= 1e9 ether,
            "you can't charge more than all the HBO in the world, greedy"
        );
    ...
    /**
     * @notice Called by HBO.transferAndCall, on successful HBO transfer
     *
     ...
     *
     * @param _sender address: who sent the HBO (must be a contract)
     * @param _fee amount of HBO sent
    ...
    /**
     * @notice creates the HashBridgeOracle request for randomness
     *
     ...
     * @param _feePaid Amount of HBO sent with request. Must exceed fee for key
    ...
    function randomnessRequest(
        ...
        uint256 _feePaid,
        address _sender
    ) internal sufficientHBO(_feePaid, _keyHash) {
        ...
        assert(_feePaid < 1e27); // Total HBO fits in uint96
    ...
    /**
     * @dev Allows the oracle operator to withdraw their HBO
     * @param _recipient is the address the funds will be sent to
     * @param _amount is the amount of HBO transferred from the Coordinator contract
     */
        ...
        hasAvailableFunds(_amount)
    {
        ...
        assert(HBO.transfer(_recipient, _amount));
    }
     ...
     */
    modifier sufficientHBO(uint256 _feePaid, bytes32 _keyHash) {
        require(
     ...
     */
    modifier onlyHBO() {
        require(msg.sender == address(HBO), "Must use HBO token");
        _;
    }
